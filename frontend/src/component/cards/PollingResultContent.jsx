...
const PollOptionVoteResult = ({ label, optionVotes, totalVotes }) => {
  ...
  return (
    <div className="w-full bg-purple-500/20  rounded-md h-6 relative mb-3">
      <div
        className="bg-purple-800/30 h-6 rounded-md"
        style={{ width: `${progress}% ` }}
      ></div>
      <span className="absolute inset-0 flex items-center justify-between text-purple-700/50 text-[12px] font-medium mx-4">
        {label} <span className="text-[11px] text-purple-800">{progress}%</span>
      </span>
    </div>
  );
};
